function export_theta(varargin)
...
for i = 1:1:lstr
    ...
    for j = 1:1:bits(end)
        ...
        rivers(k) = struct('Geometry','Line','BoundingBox',[minlon minlat maxlon maxlat],'Lon',lo(ind),'Lat',la(ind),'sid',i,'mid',j,'value',-1*stream(i).m2(j));
        %         rivers(k) = struct('Geometry','Line','BoundingBox',[minlon minlat
        %         maxlon maxlat],'Lon',stream(i).lon(ind),'Lat',stream(i).lat(ind),'sid',i,'mid',j,'m1',stream(i).m1(j));
