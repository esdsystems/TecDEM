function export_uplift(varargin)
...
for i = 1:1:lstr
    ...
    bits = unique(stream(i).bit);
        ...
        for j = 1:1:bits(end)
            ...
            K = 0.001;
            uplift = stream(i).Ks(j)*K;
            rivers(k) = struct('Geometry','Line','BoundingBox',[minlon minlat maxlon maxlat],'Lon',lo(ind),'Lat',la(ind),'sid',i,'mid',j,'value',uplift);
            k = k+1;
        end
